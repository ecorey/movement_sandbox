...
var A = {
  OperationDefinition(e) {
    if ("query" === e.operation && !e.name && !hasItems(e.variableDefinitions) && !hasItems(e.directives)) {
      return A.SelectionSet(e.selectionSet);
    }
    var t = e.operation;
    if (e.name) {
      t += " " + e.name.value;
    ...
    if (hasItems(e.variableDefinitions)) {
      ...
      t += "(" + e.variableDefinitions.map(A.VariableDefinition).join(", ") + ")";
    }
    if (hasItems(e.directives)) {
      t += " " + e.directives.map(A.Directive).join(" ");
    }
    return t + " " + A.SelectionSet(e.selectionSet);
  },
  VariableDefinition(e) {
    var t = A.Variable(e.variable) + ": " + print(e.type);
    if (e.defaultValue) {
  ...
  Field(e) {
    ...
    if (hasItems(e.arguments)) {
      var i = e.arguments.map(A.Argument);
      var r = t + "(" + i.join(", ") + ")";
      t = r.length > 80 ? t + "(\n  " + i.join("\n").replace(/\n/g, "\n  ") + "\n)" : r;
    }
    if (hasItems(e.directives)) {
    ...
    return e.selectionSet ? t + " " + A.SelectionSet(e.selectionSet) : t;
  },
  ...
  ObjectValue: e => "{" + e.fields.map(A.ObjectField).join(", ") + "}",
  ...
  Directive(e) {
    var t = "@" + e.name.value;
    if (hasItems(e.arguments)) {
      t += "(" + e.arguments.map(A.Argument).join(", ") + ")";
    }
    return t;
  },
...
function print(e) {
  return A[e.kind] ? A[e.kind](e) : "";
}

var b = new Set([ "gql", "graphql" ]);
...
var isGraphQLFunctionIdentifier = e => exports.ts.isIdentifier(e) && b.has(e.escapedText);
...
function unrollFragment(t, i, r) {
  ...
  while (exports.ts.isPropertyAccessExpression(l.parent)) {
    l = l.parent;
  }
  if (exports.ts.isVariableDeclaration(l.parent) && l.parent.initializer && exports.ts.isCallExpression(l.parent.initializer)) {
    ...
    l = l.parent.initializer;
  } else if (exports.ts.isBinaryExpression(l.parent)) {
    if (exports.ts.isPropertyAccessExpression(l.parent.right)) {
      l = l.parent.right.name;
    } else {
      l = l.parent.right;
    }
  }
  if (exports.ts.isIdentifier(l)) {
    return unrollFragment(l, i, r);
  }
...
class LRUCache {
  ...
  * rvalues() {
  ...
  set(e, t, i = {}) {
    ...
    if (void 0 === p) {
      p = 0 === this.#n ? this.#d : 0 !== this.#v.length ? this.#v.pop() : this.#n === this.#t ? this.#R(!1) : this.#n;
      this.#u[p] = e;
      this.#p[p] = t;
      this.#l.set(e, p);
      this.#h[this.#d] = p;
      ...
      if (t !== h) {
        ...
        if (o) {
          o.set = "replace";
          var c = h && this.#A(h) ? h.__staleWhileFetching : h;
          if (void 0 !== c) {
            o.oldValue = c;
          }
        }
...
var I = getDefaultExportFromCjs(_.exports);
...
var generateHashForDocument = (e, i, r, s) => {
  if (s) {
    ...
    return t.createHash("sha256").update(print(parse(n))).digest("hex");
  } else {
    var o = getSource(e, r);
    var {fragments: l} = findAllCallExpressions(o, e);
    var u = resolveTemplate(i, r, e).combinedText;
    var p = parse(u);
    var h = new Set;
    visit(p, {
      FragmentSpread: e => {
    ...
    while (v = d.shift()) {
      c.add(v);
      var g = l.find((e => e.name.value === v));
      if (!g) {
        e.project.projectService.logger.info(`[GraphQLSP] could not find fragment for spread ${v}!`);
        return;
      }
      visit(g, {
        FragmentSpread: e => {
    ...
    return t.createHash("sha256").update(print(parse(f))).digest("hex");
  }
};
...
var U = [ 52001, 52002, 52004, 52003, O, P, V, j, M ];
...
exports.getGraphQLDiagnostics = function getGraphQLDiagnostics(t, i, r) {
  ...
  var p = I(s ? a.getText() + o.map((e => print(e))).join("-") + i.version : u.join("-") + i.version);
  var h;
  if (W.has(p)) {
    h = W.get(p);
  } else {
  ...
  if (s) {
    var d = findAllPersistedCallExpressions(a, r).map((e => {
      ...
      if (g.startsWith("sha256:")) {
        var m = generateHashForDocument(r, v.arguments[0], u, exports.ts.isArrayLiteralExpression(v.arguments[1]) ? v.arguments[1] : void 0);
        if (!m) {
          return null;
        }
        if (`sha256:${m}` !== g) {
...
exports.getPersistedCodeFixAtPosition = function getPersistedCodeFixAtPosition(e, t, i) {
  ...
  var g = generateHashForDocument(i, v.arguments[0], u, exports.ts.isArrayLiteralExpression(v.arguments[1]) ? v.arguments[1] : void 0);
  var m = o.arguments[0];
  if (!m) {
    return {
      span: {
