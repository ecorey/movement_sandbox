...
function error(e) {
  return new GraphQLError(`Syntax Error: Unexpected token at ${F} in ${e}`);
}

function advance(e) {
  e.lastIndex = F;
  if (e.test(L)) {
    return L.slice(F, F = e.lastIndex);
  }
}
...
function ignored() {
  for (var e = 0 | L.charCodeAt(F++); 9 === e || 10 === e || 13 === e || 32 === e || 35 === e || 44 === e || 65279 === e; e = 0 | L.charCodeAt(F++)) {
    if (35 === e) {
      while (10 !== (e = L.charCodeAt(F++)) && 13 !== e) {}
    }
  }
  F--;
}
...
function selectionSet() {
  ...
  if (123 === L.charCodeAt(F)) {
    ...
    if (!t.length || 125 !== L.charCodeAt(F++)) {
...
function unrollFragment(e, t, i) {
  ...
  while (b.isPropertyAccessExpression(o.parent)) {
    o = o.parent;
  }
  if (b.isVariableDeclaration(o.parent) && o.parent.initializer && b.isCallExpression(o.parent.initializer)) {
    o = o.parent.initializer;
  } else if (b.isPropertyAssignment(o.parent)) {
    o = o.parent.initializer;
  } else if (b.isBinaryExpression(o.parent)) {
    if (b.isPropertyAccessExpression(o.parent.right)) {
...
function getAllFragments(e, t, i) {
  ...
  b.forEachChild(h, (e => {
    if (b.isVariableStatement(e) && e.declarationList && "documents" === e.declarationList.declarations[0].name.getText()) {
  ...
  return s = o.flatMap((s => {
    ...
    var u, _loop2 = function() {
      if (!n && (b.isReturnStatement(o) || b.isArrowFunction(o))) {
        ...
        o = o.parent;
      } else if (b.isPropertyAccessExpression(o) && ne.has(o.name.text) && b.isCallExpression(o.parent)) {
        ...
        if (b.isIdentifier(f)) {
          ...
          if (p && b.isFunctionDeclaration(p)) {
            f = p;
          } else if (p && b.isVariableDeclaration(p) && p.initializer) {
            f = p.initializer;
          }
        }
        if (b.isFunctionDeclaration(f) || b.isFunctionExpression(f) || b.isArrowFunction(f)) {
          var d = crawlScope(f.parameters[s ? 1 : 0].name, l, i, r, a, !0);
          if (b.isVariableDeclaration(h.parent) && !u) {
...
function getPersistedCodeFixAtPosition(e, t, i) {
  ...
  var v = l;
  if (!v || !b.isCallExpression(v) || !b.isStringLiteralLike(v.arguments[0])) {
    return;
  }
  var g = generateHashForDocument(i, v.arguments[0], u, b.isArrayLiteralExpression(v.arguments[1]) ? v.arguments[1] : void 0);
  var m = o.arguments[0];
  if (!m) {
    return {
      span: {
...
var generateHashForDocument = (e, t, i, r) => {
  if (r) {
    ...
    return A("sha256").update(print(parse(n))).digest("hex");
  } else {
    var s = getSource(e, i);
    var {fragments: o} = findAllCallExpressions(s, e);
    var l = resolveTemplate(t, i, e).combinedText;
    var u = parse(l);
    var h = new Set;
    visit(u, {
      FragmentSpread: e => {
    ...
    while (d = p.shift()) {
      c.add(d);
      var v = o.find((e => e.name.value === d));
      if (!v) {
        e.project.projectService.logger.info(`[GraphQLSP] could not find fragment for spread ${d}!`);
        return;
      }
      visit(v, {
        FragmentSpread: e => {
          if (!c.has(e.name.value)) {
            p.push(e.name.value);
          }
        }
      });
    ...
    return A("sha256").update(print(parse(f))).digest("hex");
  }
};
  ...
  if (r) {
    var p = findAllPersistedCallExpressions(a, i).map((t => {
      ...
      if (g.startsWith("sha256:")) {
        var m = generateHashForDocument(i, v.arguments[0], u, b.isArrayLiteralExpression(v.arguments[1]) ? v.arguments[1] : void 0);
        if (!m) {
          return null;
        }
        if (`sha256:${m}` !== g) {
